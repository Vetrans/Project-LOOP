...
const TEMPLATES = [
  ...
  {
    text: "Billing page keeps timing out when I try to download an invoice.",
    channel: "Support ticket",
  },
  {
    text: "Setup wizard is confusing, I gave up halfway through onboarding.",
    channel: "Support ticket",
  },
  {
    text: "Mobile app crashes every time I try to open the inbox.",
    channel: "App store review",
  },
  {
    text: "Would love SSO support for our security team's compliance review.",
    channel: "Sales call note",
  },
  {
    text: "Invoice totals don't match what we were quoted, very frustrating.",
    channel: "Support ticket",
  },
  {
    text: "Reporting exports are amazing now, thank you for the CSV option.",
    channel: "Community post",
  },
  {
    text: "Great product overall, onboarding docs could use more screenshots.",
    channel: "NPS survey",
  },
  {
    text: "We need role-based access before we can roll this out company-wide.",
    channel: "Sales call note",
  },
  {
    text: "The mobile experience feels like an afterthought compared to desktop.",
    channel: "App store review",
  },
  {
    text: "Support team was fast and helpful resolving my billing question.",
    channel: "Support ticket",
  },
  {
    text: "Dashboard load times have gotten noticeably slower this month.",
    channel: "NPS survey",
  },
  {
    text: "Onboarding flow saved us hours compared to our old tool.",
    channel: "Community post",
  },
  {
    text: "Still waiting on SSO — this is blocking our security sign-off.",
    channel: "Sales call note",
  },
  {
    text: "Exported report was missing several columns we needed.",
    channel: "Support ticket",
  },
  {
    text: "App store reviewers keep mentioning the app is confusing to navigate.",
    channel: "App store review",
  },
];
...
async function run() {
  ...
  for (const t of THEMES) {
    themeDocs[t.name] = await Theme.create({
      ...t,
  ...
  console.log(
    "[seed] generating 130 feedback items (this classifies each one — may take a bit)…",
  );
  let created = 0;
  for (let i = 0; i < 130; i++) {
    ...
    const classification = classifyFeedback(content, Object.keys(themeDocs));
  ...
  console.log(
    `[seed] done — ${created} feedback items, ${THEMES.length} themes, 3 users.`,
  );
